...
firebase.auth().onAuthStateChanged((user)=>{
    if(!user){
        ...
        var nameInput = document.getElementById('First-Name-input')
        let FirstnameT=document.getElementById("FirstnameT");
        database.ref('users/' + user.uid +'/template1'+'/Personal-data'+'/firstname').once('value')
        .then((snapshot) => {
...
database.ref('users/'+user.uid+'/template1'+'/Template-Editor').child('Color').set(colorchoice.value);
database.ref('users/' + user.uid + '/template1'+'/Personal-data').once('value')
      .then((snapshot) => {
